import { useEffect, useRef, useState } from "react";
import Plotly from "plotly.js-dist-min";
import * as api from "../api/client.js";
import { SC, SI, SENSES } from "../lib/constants.js";

function scoreRgb(v) {
  if (v == null) return "240,237,232";
  return v < 0.5 ? "232,131,106" : v < 0.65 ? "212,185,106" : "139,184,138";
}

function centroid(poly) {
  const pts = poly.length > 1 && poly[0][0] === poly[poly.length-1][0] && poly[0][1] === poly[poly.length-1][1]
    ? poly.slice(0, -1) : poly;
  let x = 0, y = 0;
  pts.forEach(p => { x += p[0]; y += p[1]; });
  return [x / pts.length, y / pts.length];
}

export default function LayoutPlan({
  rooms, selectedRoom, onSelect, layoutId,
  layoutDiff, biophilicData, showMode,  // showMode: "comfort" | "biophilic"
}) {
  const ref      = useRef(null);
  const [layout, setLayout] = useState(null);
  const [err,    setErr]    = useState("");
  const [pulsingRoom, setPulsingRoom] = useState(null);

  // Fetch layout geometry
  useEffect(() => {
    let alive = true;
    api.getLayout()
      .then(d => { if (alive) { setLayout(d.layout || null); if (!d.layout) setErr("no layout loaded yet"); } })
      .catch(() => { if (alive) setErr("could not load layout"); });
    return () => { alive = false; };
  }, [layoutId]);

  // Pulse changed room when layout_diff arrives
  useEffect(() => {
    if (!layoutDiff || !layoutDiff.room_id) return;
    setPulsingRoom(layoutDiff.room_id);
    const t = setTimeout(() => setPulsingRoom(null), 2800);
    return () => clearTimeout(t);
  }, [layoutDiff]);

  // Render Plotly
  useEffect(() => {
    if (!ref.current || !layout) return;

    // Build lookup maps
    const scoreByName = {};
    const conflictsByName = {};     // roomName → Set of failing senses
    const bioByName = {};           // roomName → richness score
    const diffRoomName = layoutDiff?.room_name;

    (rooms || []).forEach(r => {
      scoreByName[r.roomName] = r.overallScore;
      const failing = new Set(
        Object.entries(r.comfortScores || {}).filter(([,v]) => v < 0.5).map(([s]) => s)
      );
      conflictsByName[r.roomName] = failing;
    });

    if (biophilicData?.rooms) {
      biophilicData.rooms.forEach(r => { bioByName[r.name] = r.richness_score; });
    }

    const traces = [];
    const annotations = [];

    // Room polygons
    (layout.rooms || []).forEach(room => {
      const geo     = room.geometry || [];
      if (geo.length < 3) return;
      const name    = room.name;
      const isSel   = name === selectedRoom;
      const isDiff  = name === diffRoomName;

      let score;
      if (showMode === "biophilic") {
        score = bioByName[name];
      } else {
        score = scoreByName[name];
      }

      const rgb     = scoreRgb(score);
      const fillOp  = isDiff ? 0.65 : isSel ? 0.42 : 0.20;
      const lineOp  = isDiff ? 1.00 : isSel ? 0.95 : 0.55;
      const lineW   = isDiff ? 3.5  : isSel ? 2.5  : 1.2;

      traces.push({
        x: geo.map(p => p[0]), y: geo.map(p => p[1]),
        mode: "lines", fill: "toself",
        fillcolor: `rgba(${rgb},${fillOp})`,
        line: { color: `rgba(${rgb},${lineOp})`, width: lineW },
        name, text: name, hoverinfo: "text",
        customdata: [name],
      });

      // Labels: score number + sense badges directly on the room
      const [cx, cy] = centroid(geo);
      const scoreLabel = score != null ? score.toFixed(2) : "";
      const failing    = conflictsByName[name] || new Set();
      const badges     = [...failing].map(s => SI[s] || s.slice(0,3)).join(" ");

      annotations.push({
        x: cx, y: cy + 0.25,
        text: scoreLabel,
        showarrow: false,
        font: { family: "IBM Plex Mono, monospace", size: 11, color: `rgba(${rgb},0.95)` },
      });
      if (badges) {
        annotations.push({
          x: cx, y: cy - 0.30,
          text: badges,
          showarrow: false,
          font: { family: "IBM Plex Mono, monospace", size: 8, color: `rgba(${rgb},0.70)` },
        });
      }

      // Diff badge
      if (isDiff && layoutDiff) {
        annotations.push({
          x: cx, y: cy - 0.80,
          text: `${layoutDiff.old_value} → ${layoutDiff.new_value}`,
          showarrow: false,
          font: { family: "IBM Plex Mono, monospace", size: 7, color: "rgba(240,237,232,0.80)" },
          bgcolor: "rgba(30,30,30,0.70)",
          borderpad: 3,
        });
      }
    });

    // Walls (dark, thick)
    (layout.structure || []).forEach(w => {
      const g = w.geometry || [];
      if (g.length < 2) return;
      traces.push({ x: g.map(p=>p[0]), y: g.map(p=>p[1]), mode:"lines", line:{color:"rgba(20,20,20,0.95)",width:3}, hoverinfo:"skip", showlegend:false });
    });
    // Doors (warm accent)
    (layout.doors || []).forEach(d => {
      const g = d.geometry || [];
      if (g.length < 2) return;
      traces.push({ x: g.map(p=>p[0]), y: g.map(p=>p[1]), mode:"lines", line:{color:"rgba(232,131,106,0.75)",width:2.5}, hoverinfo:"skip", showlegend:false });
    });
    // Windows (teal)
    (layout.windows || []).forEach(wn => {
      const g = wn.geometry || [];
      if (g.length < 2) return;
      traces.push({ x: g.map(p=>p[0]), y: g.map(p=>p[1]), mode:"lines", line:{color:"rgba(106,184,200,0.90)",width:3}, hoverinfo:"skip", showlegend:false });
    });
    // Furniture (subtle)
    (layout.furniture || []).forEach(f => {
      const g = f.geometry || [];
      if (g.length < 3) return;
      traces.push({ x: g.map(p=>p[0]), y: g.map(p=>p[1]), mode:"lines", line:{color:"rgba(139,184,138,0.40)",width:1}, hoverinfo:"skip", showlegend:false });
    });

    const plLayout = {
      margin: { l:8, r:8, t:8, b:8 },
      paper_bgcolor: "rgba(0,0,0,0)",
      plot_bgcolor:  "rgba(0,0,0,0)",
      xaxis: { visible:false, scaleanchor:"y", scaleratio:1 },
      yaxis: { visible:false },
      showlegend: false,
      annotations,
    };

    Plotly.react(ref.current, traces, plLayout, { displayModeBar:false, responsive:true });

    const div = ref.current;
    const handler = e => {
      const pt   = e.points?.[0];
      const name = pt?.data?.customdata?.[0];
      if (name && onSelect) onSelect(name);
    };
    div.on?.("plotly_click", handler);
    return () => { try { div.removeAllListeners?.("plotly_click"); } catch {} };
  }, [layout, rooms, selectedRoom, layoutDiff, biophilicData, showMode, onSelect]);

  if (err) return <div className="ap-empty">{err}</div>;
  return (
    <div style={{ position:"relative", width:"100%", height:"100%" }}>
      <div ref={ref} style={{ width:"100%", height:"100%" }} />
      {/* Pulse ring overlay on changed room */}
      {pulsingRoom && (
        <div className="layout-pulse-overlay" key={pulsingRoom}>
          <div className="layout-pulse-ring" />
        </div>
      )}
    </div>
  );
}
