...
export default function GraphEdge({
  ...
  glow = true, curvature = 0.18, opacity = 0.85, live = false, onHover, onLeave,
}) {
  ...
  if (arrow && headSize > 0) {
    ...
    head = <polygon points={`${bx},${by} ${b1x},${b1y} ${b2x},${b2y}`} fill={color} />;
  }
  return (
    <g opacity={opacity} onMouseMove={onHover} onMouseLeave={onLeave} style={onHover ? { cursor: "help" } : undefined}>
      {glow && <path d={a.d} fill="none" stroke={color} strokeWidth={width * 3} strokeOpacity={0.16} strokeLinecap="round" vectorEffect="non-scaling-stroke" />}
      <path className={"graph-edge" + (live ? " is-live" : "")} d={a.d} fill="none" stroke={color}
        strokeWidth={width} strokeDasharray={live ? undefined : dash} strokeLinecap="round" vectorEffect="non-scaling-stroke" />
      {head}
      {onHover && <path d={a.d} fill="none" stroke="transparent" strokeWidth={Math.max(width * 4, 10)} vectorEffect="non-scaling-stroke" />}
    </g>
  );
}
