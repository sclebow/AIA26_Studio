...
- [x] **Straight grid + function-driven orientation (current)**: on `derive_site_grid` (lines parallel/perpendicular to a chosen side), `place_building_by_function` rotates a footprint to a grid-aligned orientation by **use** (`FUNCTION_FRONTAGE`: commercial/retail → long side parallel/frontage; residential/office → perpendicular/deep), keeping the shape exact. The agent asks the user the function. Rectilinear shapes sit perfectly square; diagonal-armed Y/X/O align by their dominant axis. (The earlier warped-grid `conform_building_to_grid` / full-rubber-sheet attempts were rejected for distorting shapes; they remain in the backend but are unused by the notebooks.)
