...
def _retransform_saved_plates(plates: list, old_b: list, new_b: list) -> list:
    """Move/rotate/scale every floor-plate ring (footprint + holes + wing_footprints) by the
    rigid transform that took the original boundary `old_b` to the variant's placed boundary
    `new_b`. The viewer renders the PLATES, so without this the optimized variants drew their
    floor stacks at the ORIGINAL position regardless of the optimized placement (different
    score, same place). Transform = translate old centroid→new centroid, rotate by the change
    in longest-edge bearing, scale by the size ratio — same approach as the manipulation route."""
    import math as _m

    if not plates or not old_b or not new_b or len(old_b) < 3 or len(new_b) < 3:
        return [dict(p) for p in (plates or [])]
    ...
    def _xf(q):
        ...
        return [ncx + (x * ca - y * sa), ncy + (x * sa + y * ca)]
    ...
    for p in plates:
        ...
        if isinstance(p.get("holes"), list) and p["holes"]:
            np["holes"] = [[_xf(q) for q in ring] for ring in p["holes"]]
        if isinstance(p.get("wing_footprints"), dict):
            np["wing_footprints"] = {k: [_xf(q) for q in ring] for k, ring in p["wing_footprints"].items()}
        out.append(np)
    return out
...
async def optimize_all_saved(session_id: str, body: OptimizeSavedRequest | None = None) -> dict[str, Any]:
    ...
    def _placement_score(boundary):
        """A REAL, position-derived 0..100 score for WHERE the footprint sits on the
        site: how balanced/central it is (open space on all sides) blended with how far
        it stands off the site edges. Same shape at two positions therefore scores
        differently — without this, an empty site (no urban context) made every placement
        of a shape tie on the shape-only objectives, so variant A and B looked identical.
        This is geometry, not a fabricated number: a centred, well-set-back placement
        genuinely has more usable surrounding open space than an edge-hugging one."""
        try:
            if not boundary or len(boundary) < 3 or not site_boundary:
                return None
            bxs = [p[0] for p in boundary]; bys = [p[1] for p in boundary]
            sxs = [p[0] for p in site_boundary]; sys_ = [p[1] for p in site_boundary]
            bcx = sum(bxs) / len(bxs); bcy = sum(bys) / len(bys)
            scx = sum(sxs) / len(sxs); scy = sum(sys_) / len(sys_)
            sw = max(sxs) - min(sxs) or 1.0; sh = max(sys_) - min(sys_) or 1.0
            # 1) centrality: 1 at the site centre, →0 toward an edge.
            offx = abs(bcx - scx) / (sw / 2); offy = abs(bcy - scy) / (sh / 2)
            centrality = max(0.0, 1.0 - (offx + offy) / 2)
            # 2) min edge gap (normalised): the smallest clearance to any site edge.
            gaps = [min(bxs) - min(sxs), max(sxs) - max(bxs),
                    min(bys) - min(sys_), max(sys_) - max(bys)]
            min_gap = max(0.0, min(gaps))
            gap_norm = min(1.0, min_gap / (0.15 * min(sw, sh)))   # full credit at ~15% of the short side
            return round(100 * (0.6 * centrality + 0.4 * gap_norm), 1)
        except Exception:  # noqa: BLE001
            return None

    def _variant_metrics(sc, footprint, fl, boundary=None):
        """Full objective set + FAR/coverage/floors/design-health for ONE variant.
        Folds in a position-derived `placement` score so same-shape variants at different
        spots get genuinely different totals (the previous behaviour tied them at one
        number, making A/B look identical)."""
        os = sc["objective_scores"]
        m = {k: round(v, 1) for k, v in os.items()}          # ALL available objectives
        health = sc["weighted_score"]
        m["design_health"] = round(health, 1)
        place = _placement_score(boundary)
        if place is not None:
            m["placement"] = place
            # Blend placement into the total (small, honest weight) so position matters.
            m["total"] = round(0.8 * health + 0.2 * place, 1)
        else:
            m["total"] = round(health, 1)
        m["far"] = round((footprint * fl) / site_area, 3) if site_area else None
        m["coverage"] = round(footprint / site_area, 3) if site_area else None
        m["floors"] = fl
        return m

    # Minimum separation between variants of ONE shape, scaled to the site so the
    # difference is VISIBLE in the preview (variant A vs B were landing ~15 m apart on a
    # ~90 m site — a real but imperceptible shift, so they read as "same location"). Use
    # ~30% of the site's short dimension, floored at 20 m, so A and B are obviously apart.
    _site_short = _site_min_dim(site_boundary) or 60.0
    _MIN_SEP = max(20.0, 0.30 * _site_short)

    def _distinct(pl, chosen):
        """True if this placement differs ENOUGH from already-chosen ones to be visibly
        different in the preview — either far apart (≥ _MIN_SEP) or clearly re-rotated
        (≥ 25°). Without a real separation, variant B looked identical to A on the site."""
        import math as _m
        for c in chosen:
            dist = _m.hypot(pl["centroid_xy"][0] - c["centroid_xy"][0],
                            pl["centroid_xy"][1] - c["centroid_xy"][1])
            dr = abs((pl.get("rotation_degrees", 0) - c.get("rotation_degrees", 0)) % 360)
            rotated = 25 <= dr <= 335
            if dist < _MIN_SEP and not rotated:
                return False
        return True
    ...
    for s_idx, opt in enumerate(saved):
        ...
        if not placements:
            ...
            try:
                ...
                # If the AS-DRAWN footprint pokes OUTSIDE the site, the optimized option
                # would overlap the boundary (the bug). Re-CENTER it on the site centroid
                # first, so the fallback's base position is always inside the plot.
                if not _site_poly.contains(_P(sample_boundary)):
                    scx, scy = _site_poly.centroid.x, _site_poly.centroid.y
                    base_off = (scx - cx0, scy - cy0)
                cand_positions = [base_off]
                bx, by = base_off
                for ddx, ddy in ((8, 0), (-8, 0), (0, 8), (0, -8), (6, 6), (-6, -6)):
                    moved = [[p[0] + bx + ddx, p[1] + by + ddy] for p in sample_boundary]
                    if _site_poly.contains(_P(moved)):
                        cand_positions.append((bx + ddx, by + ddy))
        ...
        for cand in scored:
            ...
            if _distinct(cand["pl"], [c["pl"] for c in chosen]):
                chosen.append(cand)
        # POSITION-ONLY collapse: the user judges "same place" by LOCATION, not rotation.
        # On a tight site the only second variant is the SAME spot rotated — which looks
        # identical in the preview ("2 options, different score, same place"). So drop any
        # later variant whose centroid is within the visible-separation threshold of an
        # already-kept one, REGARDLESS of rotation. Result: a roomy site still gives 2
        # genuinely different placements; a tight site honestly returns just ONE.
        deduped: list[dict[str, Any]] = []
        for cand in chosen:
            cxy = cand["pl"]["centroid_xy"]
            too_close = any(
                ((cxy[0] - k["pl"]["centroid_xy"][0]) ** 2
                 + (cxy[1] - k["pl"]["centroid_xy"][1]) ** 2) ** 0.5 < _MIN_SEP
                for k in deduped
            )
            if not too_close:
                deduped.append(cand)
        chosen = deduped
        # NOTE: NO "fill with next-best regardless" fallback — that re-introduced the
        # same-place near-duplicates. Always keep at least the single best placement.
        if not chosen and scored:
            chosen = [scored[0]]
        src = opt["shape_option_id"]
        orig_plates = geo.get("floor_plates") or []
        for v_idx, cand in enumerate(chosen):
            pl, sc = cand["pl"], cand["sc"]
            footprint = _poly_area(pl["boundary"])
            tag = chr(65 + v_idx)                       # A, B, ...
            # CRITICAL: the 3D viewer renders the FLOOR PLATES, not the boundary. Each
            # variant's boundary was moved/rotated to its placement, but the plates were
            # being copied UN-MOVED from the original — so every variant's floor stack drew
            # at the SAME original spot ("2 options, different score, same place", even on a
            # big site). Transform the plates by the SAME boundary→placement transform so the
            # rendered massing actually sits at the variant's location.
            placed_plates = _retransform_saved_plates(orig_plates, boundary, pl["boundary"])
            results.append({
                ...
                "geometry": {"boundary": pl["boundary"], "holes": opt_holes,
                             "floor_plates": placed_plates, "floors": floors},
                "placement": {
                    "position": [round(pl["centroid_xy"][0], 1), round(pl["centroid_xy"][1], 1), 0],
                    "rotation_degrees": pl.get("rotation_degrees", 0),
                    "scale": 1.0,
                },
                "scores": _variant_metrics(sc, footprint, floors, boundary=pl["boundary"]),
    ...
    for opt in saved:
        ...
        try:
            ...
            source_shapes.append({
                ...
                "is_baseline": True,
                "scores": _variant_metrics(sc, fp, fl, boundary=b),
                "objective_scores": sc["objective_scores"],
