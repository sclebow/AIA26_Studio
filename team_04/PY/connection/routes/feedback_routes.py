...
def _translate_plates(plates: list[dict[str, Any]], dx: float, dy: float) -> list[dict[str, Any]]:
    """Shift every plate by (dx, dy) — BOTH its footprint AND its per-wing footprints.
    The place_center / place_corner / place_edge moves used to translate only `footprint`,
    leaving `wing_footprints` at the old spot; a later 'add floors on <wing>' then placed
    the new floor at the stale location (the misaligned block). Translating both keeps the
    wings welded to the moved building."""
    out = []
    for p in plates:
        np = dict(p)
...
def _retransform_plates(plates: list[dict[str, Any]], old_b: list, new_b: list) -> list[dict[str, Any]]:
    ...
    for p in plates:
        np = dict(p)
        np["footprint"] = [
            [ncx + (q[0] - ocx) * scale, ncy + (q[1] - ocy) * scale]
            for q in p["footprint"]
        ]
        if "wing_footprints" in p:
            np["wing_footprints"] = {
                r: [[ncx + (q[0] - ocx) * scale, ncy + (q[1] - ocy) * scale] for q in ring]
                for r, ring in p["wing_footprints"].items()
            }
        out.append(np)
    return out
    ...
    for p in parsed:
        ...
        if op in ("place_corner", "place_edge"):
            ...
            if not r.get("ok"):
                ...
                continue
            if not _ring_fits_site(r["outer"], site_boundary):
                applied_results.append({"text": text, "op": op, "accepted": False,
                                        "reason": "the building doesn't fit at that corner with the setback",
                                        "suggestion": "make the building smaller first"})
                continue
