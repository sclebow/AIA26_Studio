...
export async function sendMessage(message, action = null) {
  ...
  if ((action?.intent === "optimize" || /^optimize/i.test(message || "")) && !action?.optimizeScope) {
    ...
    if (ticked.length) {
      ...
      pushMessage("assistant", `⚙ Optimizing **${ticked.length} selected shape${ticked.length > 1 ? "s" : ""}** — generating 2 variants each…`);
      try {
        ...
        if (_nVar > 0) {
          pushMessage("assistant",
            `Done — **${_nVar} optimized variants** are in the Optimization Explorer (each saved shape → 2 placements) and ready to compare with their originals.`,
            [{ label: "Compare", message: "compare", intent: "compare" }]);
        } else {
          pushMessage("assistant",
