...
function drawSetback(boundary) {
  // Default residential-ish setback margin (metres). Visual guide; the backend enforces
  // the real per-type rules during validation/placement.
  const span = (() => { const xs = boundary.map((p) => p[0]), ys = boundary.map((p) => p[1]);
    return Math.min(Math.max(...xs) - Math.min(...xs), Math.max(...ys) - Math.min(...ys)); })();
  const margin = Math.max(3, Math.min(span * 0.12, 9));   // 3–9 m, ~12% of the short side
...
function drawOption(opt, color, opacity = 0.95, selectable = true) {
  ...
  if (Array.isArray(plates) && plates.length) {
    ...
    for (let i = 0; i < sorted.length; i++) {
      ...
      const holes = _holesInside(opt.holes, fp);
