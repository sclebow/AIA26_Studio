...
const NA = Symbol("N/A");
const METRICS = [
  // Strategy only applies to named optimization strategies; placement variants have
  // none → return NA so the all-NA row gets hidden (no useless row of "—").
  ["Strategy", (o) => o.strategy_name || NA, true, (v) => v, { text: true }],
  ...
  // Footprint: use the stored area, else compute it from the boundary/footprint ring so
  // optimized variants (which carry geometry but not a pre-set footprint_area) show a real
  // number instead of N/A — they ARE the same shape, just repositioned.
  ["Footprint", (o) => num(o.footprint_area) || ringArea(o.boundary || o.footprint) || NA, true,
...
// Shoelace area of a polygon ring (m²) — used so variants with geometry but no stored
// footprint_area still report a real footprint instead of N/A.
function ringArea(ring) {
  if (!Array.isArray(ring) || ring.length < 3) return 0;
  let a = 0;
  for (let i = 0; i < ring.length; i++) {
    const j = (i + 1) % ring.length;
    a += Number(ring[i][0]) * Number(ring[j][1]) - Number(ring[j][0]) * Number(ring[i][1]);
  }
  return Math.abs(a) / 2;
}
...
function render(ids) {
  ...
  // optimizer doesn't compute, AND Strategy for placement variants) just adds noise. Only
  // "Overall Score" is always kept; everything else (incl. Strategy) is dropped when no
  // column has a value. Honest: a hidden row = a metric produced for nobody, not faked.
  const ALWAYS = new Set(["Overall Score"]);
