...
def _extract_tree_payload(state: dict[str, Any]) -> dict[str, Any]:
    ...
    tree_points = planning_context.get("tree_points", tree_policy.get("tree_points", tree_policy.get("inferred_tree_points", [])))
    tree_sizes = planning_context.get("tree_sizes", tree_policy.get("tree_sizes", tree_policy.get("inferred_tree_sizes", [])))
        ...
        for pending_tool in pending_tools:
            ...
            if _should_inject_tree_payload(tool_name, filtered_args):
                if shape_payload["shape_type"]:
                    filtered_args.setdefault("shape_type", shape_payload["shape_type"])
                if shape_payload["locked_shape_type"]:
                    filtered_args.setdefault("locked_shape_type", shape_payload["locked_shape_type"])
                ...
                filtered_args.setdefault("tree_policy", {})
                if isinstance(filtered_args.get("tree_policy"), dict):
                    filtered_args["tree_policy"].setdefault("tree_points", tree_payload["tree_points"])
                    filtered_args["tree_policy"].setdefault("tree_sizes", tree_payload["tree_sizes"])
                ...
                for payload_key in ("genes_json", "genes", "design_request"):
                    ...
                    if isinstance(payload_value, dict):
                        if shape_payload["shape_type"]:
                            payload_value.setdefault("shape_type", shape_payload["shape_type"])
                        if shape_payload["locked_shape_type"]:
                            payload_value.setdefault("locked_shape_type", shape_payload["locked_shape_type"])
                        ...
                        payload_value.setdefault("tree_policy", {})
                        if isinstance(payload_value.get("tree_policy"), dict):
                            payload_value["tree_policy"].setdefault("tree_points", tree_payload["tree_points"])
                            payload_value["tree_policy"].setdefault("tree_sizes", tree_payload["tree_sizes"])
                        ...
                        if isinstance(payload_data, dict):
                            if shape_payload["shape_type"]:
                                payload_data.setdefault("shape_type", shape_payload["shape_type"])
                            if shape_payload["locked_shape_type"]:
                                payload_data.setdefault("locked_shape_type", shape_payload["locked_shape_type"])
                            ...
                            payload_data.setdefault("tree_policy", {})
                            if isinstance(payload_data.get("tree_policy"), dict):
                                payload_data["tree_policy"].setdefault("tree_points", tree_payload["tree_points"])
                                payload_data["tree_policy"].setdefault("tree_sizes", tree_payload["tree_sizes"])
