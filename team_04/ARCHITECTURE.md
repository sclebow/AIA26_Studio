...
- **Buildings stay rigid.** The warped grid is used as an **orientation field**, not a rubber sheet: `align_building_to_local_grid` rotates a footprint to the local grid direction at its node, preserving the **exact shape, area, and vertex count** for every library shape (I/L/T/U/H/Y/X/O). On a warped grid the local direction varies by location, so the layout adapts while every building stays realistic; an L can additionally bend a free wing (`corner_wing_rotation`) to span an obtuse corner. (An earlier footprint-*conforming* experiment — `conform_polygon_to_grid` / `conform_world_footprint_to_grid` / the `*_region_in_*` authoring helpers / `grid_world_mapper` — pushed the whole polygon through the Coons patch and was **removed**: it rubber-sheeted complex shapes like X/Y and blew up their area 4–9×, which no real building does.) The building thus *deforms* to fit the plot, and manipulation (move / stretch / reshape) happens in `(s, t)` / cell space with the world footprint re-conforming automatically. On a rectangle the map is affine, so edges stay straight — conforming is a strict generalisation of rigid placement. Internals: `_coons_inputs`/`_coons_eval` are shared by the grid and the mapper. `_select_quad_corners` **always anchors the bottom chain on the chosen alignment side** (the other two corners split the opposite arc into thirds), so re-keying the grid to a different side actually moves the grid — and any building on it — to follow that side, rather than drifting to whichever corners are sharpest.
